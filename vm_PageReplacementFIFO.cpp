...
void push_in_queue(int* queue, int factor) {
	for (int i = 0; i < 128; i++) {
		if (queue[i] == -1) {
			queue[i] = factor;
			break;
...
int pushAndpop(int* queue, int factor) {
	int head = queue[0];
	for (int i = 1; i < 128; i++) {
		queue[i - 1] = queue[i];
	}
...
int main() {
	...
	int pageNumqueue[128];
	memset(pageNumqueue, -1, sizeof(pageNumqueue));//在FIFO Page Replacement中记录页面访问的队列，先进先出
		...
		if (inTLB(pageNum, myTLB)) {//判断是否在TLB中
			...
			if (inPageTable(pageNum, PageTable)) {//如果不在TLB中，判断是否在页表中
				...
				if (!PageTableisFull(PageTable)) {//如果页表没满
					push_in_queue(pageNumqueue, pageNum);//把正在访问的页码压进队列
					...
					int oldPage = pushAndpop(pageNumqueue,pageNum);//需要置换掉的是访问队列出来的第一个
					frame = readFromDisk(Physical_Mem, pageNum, &PageTable[oldPage]);
					PageTable[oldPage] = frame;//更新页表，并进行FIFO置换
				}
